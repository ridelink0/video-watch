...
|---|---|---|
| `--n N` | `24` | how many frames to extract. Capped to the number of distinct frames actually available in the selected range at the source fps. |
| `--mode grid\|scene` | `grid` | `grid` samples evenly across the range. `scene` detects cuts (ffmpeg's `scene` filter) and keeps every one it finds, then fills the rest of `--n` with evenly spaced frames from the largest remaining gaps - so a slowly changing clip with only a couple of hard cuts still gets full coverage instead of two frames. Falls back to `grid` (with a message) if fewer than 2 cuts are found. |
...
| `--from T` / `--to T` | clip start / end | restrict extraction to a time range. Accepts plain seconds (`90`, `12.5`) or a clock value (`mm:ss` or `hh:mm:ss`, e.g. `1:30` = 90s). An explicit `0` is honored (not read as "unset"). |
| `--threshold F` | `0.3` | scene-change sensitivity for `--mode scene` (ffmpeg's `scene` score, 0-1). An explicit `0` is honored. |
| `--sheet CxR` | off | also tile frames into contact sheets, `C` columns by `R` rows per sheet - read these first, they're far cheaper than reading every frame individually. |
...
(including an explicit `0` not being swallowed), `mm:ss` time parsing, scene
mode's cut-plus-fill top-up and its honest fallback to `grid`, and rotation
handling (coded vs. display size, portrait output).
